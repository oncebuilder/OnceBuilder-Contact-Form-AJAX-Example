...
class once extends core{
	function validate_email(){
		...
		if (is_bool($atIndex) && !$atIndex){
			$this->setError('Email without @ ?');
		}
		if ($isValid && !(checkdnsrr($domain,"MX") || checkdnsrr($domain,"A"))){
			$this->setError('Domain not found in DNS');
		}
		...
		if($this->once_csrf_token_check($this->data['csrf_token'])){
			...
			if($this->data['recaptcha_contact']){
				...
				if ($resp != null && $resp->success) {
					...
					$this->setError('Recaptcha response is wrong, check if it configured properly');
					$this->setError($error);
				}
			}
			...
			if($this->data['contact_mailbox']==1 && $this->error==0){
				...
				if(!$this->item['id']){
					...
					$this->setError('Can not insert item to: edit_mailbox');
				}
			}

			if($this->data['contact_mailme']==0 && $this->data['contact_mailbox']==0){
				$this->setError('Your contact variables $_CONFIG[contact_mailme] and $_CONFIG[contact_mailbox] are both not configured');
			}
		}else{
			$this->setError('CSFR token invalid');
		}
		// onceResponse from core.php
		$this->onceResponse();
	}
}
?>